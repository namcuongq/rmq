...
func CreateQueue(redisHost, redisPort, redisPassword string, redisDB int64, name string) *Queue {
	...
	return CreateQueueWithRedisClient(redisClient, name)
}

func CreateQueueWithRedisClient(redisClient *redis.Client, name string) *Queue {
	return newQueue(redisClient, name)
}
...
func SelectQueue(redisHost, redisPort, redisPassword string, redisDB int64, name string) (queue *Queue, err error) {
	...
	return newQueue(redisClient, name), nil
}

func newQueue(redisClient *redis.Client, name string) *Queue {
	q := &Queue{Name: name}
	q.redisClient = redisClient
...
func (queue *Queue) Put(name, payload string) error {
	p := &Package{CreatedAt: time.Now(), TaskName: name, Payload: payload, Queue: queue}
